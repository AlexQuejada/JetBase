...
const ChartRenderer: React.FC<ChartRendererProps> = ({ data, config }) => {
  ...
  if (chartData.length === 0) {
    return (
      <div className="bg-yellow-50 rounded-lg p-8 text-center">
        ...
        <p className="text-yellow-700">No hay datos para mostrar</p>
        <p className="text-sm text-yellow-600 mt-1">Verifica que la columna tenga valores</p>
      </div>
    );
  }
        ...
        return (
          <ResponsiveContainer width="100%" height={300}>
            <BarChart data={chartData}>
              ...
              <XAxis dataKey="name" />
              <YAxis />
              <Tooltip formatter={(value) => [String(Number(value || 0).toLocaleString()), config.metric]} />
        ...
        return (
          <ResponsiveContainer width="100%" height={300}>
            <PieChart>
              ...
              <Tooltip formatter={(value) => String(Number(value || 0).toLocaleString())} />
            </PieChart>
          </ResponsiveContainer>
        );
            ...
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  <th className="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase">{config.metric}</th>
                  <th className="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase">Conteo</th>
                </tr>
              </thead>
              <tbody className="bg-white divide-y divide-gray-200">
                {chartData.map((row: any, idx: number) => (
                  <tr key={idx}>
                    ...
                    <td className="px-4 py-2 text-sm text-gray-900">{Number(row.value).toLocaleString()}</td>
  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-4">
      <h4 className="text-lg font-semibold mb-4">{config.title}</h4>
      {renderChart()}
